...
//Display Char
void TM1650::displayChar(uint8_t Bit, char character) {
  int8_t segmentData;

  if (character >= '0' && character <= '9') {
    segmentData = NUM[character - '0'];  // Use NUM array for digits
  } else if (character >= 'A' && character <= 'F') {
    segmentData = CHARACTERS[character - 'A'];  // Use CHARACTERS array for A-F
  } else {
    segmentData = 0x00;  // Fallback: clear segment if character is not recognized
  }
